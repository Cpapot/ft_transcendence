...
export default async function initialize(wrapper) {
    const render = () => {
        return (`
            <div class="row justify-content-start align-items-center border rounded p-3 bg-white flex-column" style="width: 470px; height: auto;">
                <div class="row align-items-center p-3" style="font-size: 20px;" key="multi_factor_authentication"></div>
                <div class="row align-items-center p-3 border-top" style="font-size: 12px; margin-top: 20px;" key="qr_text"></div>
                <div class="row justify-content-center align-items-center" id="qrBox" style="width: 175px; height: 175px; margin-top: 20px;"></div>
                ...
                <button class="row btn btn-primary d-flex justify-content-around align-items-center" style="margin-top: 25px; font-size: 13px; width: 360px; height: 60px;" key="submit_button" disabled></button>
                <button class="row btn d-flex justify-content-around align-items-center" style="margin-top: 15px; font-size: 13px; width: 360px; height: 60px; background-color: #D3D3D3;" key="cancel_button"></button>
            </div>
        `);
    };
    ...
    const qrBox = wrapper.querySelector('#qrBox');

    cancelButton.addEventListener('click', () => {
        window.route.loadView("/profile");
    });
    ...
    const otpURL = localStorage.getItem('TEMPOtp');
    if (otpURL) {
        new QRCode(qrBox, {
            text: otpURL,
            width: 150,
